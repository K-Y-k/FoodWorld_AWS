...
- 백엔드 웹 개발자가 되기 위해 Jdbc, DB, Servlet의 기초를 다지고 
  ...
  Spring과 관련된 Spring 기초, HTTP 네트워크, MVC 패턴, DB 연결, JPA, SpringDataJPA, queryDsl을 학습한 후 배운 기술을 저의 것으로 만들기 위해 처음 구현해 본 프로젝트입니다.

- 사람에게 중요한 요소 중의 하나인 음식을 타깃으로 선정하고 공감대를 형성하기 좋은 음식의 주제로 소통을 할 수 있고 음식 메뉴를 선정할 때 도움을 제공하는 서비스로
...
## <b id="3"> 🔑 E-R 다이어그램 </b>
<img src="https://github.com/user-attachments/assets/7170dd0d-e562-46df-9ed4-50ea8170ba76" width="800" height="500"/>
...
## <b id="6"> ✔️ 주요 기능 </b>
- ### 회원가입 / 유효성 검사
  - 닉네임, 아이디, 비밀번호는 <b>글자 수 제한, 문자열 내 빈 공백 여부를 확인</b>합니다.
  - 닉네임, 아이디는 <b>추가로 중복의 유효성 검사</b>가 진행되어야 회원가입이 됩니다.
 ...
 - ### 게시판 메인 페이징, 무한 스크롤 조회 / 메뉴 카테고리 조회 / 검색
   ...
   - 먹스타그램의 메인은 <b>무한 스크롤</b>로 구현하였습니다.
 ...
 - ### 게시글 등록, 파일 업로드
   - 게시글의 등록은 <b>각 게시판에 맞는 구성과 유효성 검사를 적용</b>하였고 <b>다중 파일 업로드</b>가 가능합니다.
 ...
 - ### 게시글 상세 조회 / 좋아요 / 파일 다운로드
   - 등록한 <b>게시글을 조회</b>할 수 있고 <b>좋아요, 파일 다운로드</b>가 가능합니다.
   - 좋아요 기능은 각 게시글에 <b>회원 당 1번만 적용이 가능</b>하고 <b>취소</b>할 수도 있습니다.
 ...
 - ### 게시글 수정 / 삭제
   - 게시글 수정은 <b>연관 편의 메서드 방식</b>으로 수정하였고, <b>파일 부분은 기존 파일 삭제하고 새로 받은 파일로 재 생성</b>합니다.
   - 게시글 삭제도 DB 삭제뿐만이 아닌 <b.실제 파일도 찾아 삭제 처리</b>합니다.
 ...
 - ### 댓글 조회 / 등록 / 수정 / 삭제
   - 댓글을 <b>페이징 된 형식으로 조회</b>하고 작성한 회원의 댓글을 <b>수정, 삭제</b>할 수 있습니다.
   - 댓글 <b>수정 중에 다른 댓글로 수정하면 기존 수정 댓글 폼을 취소</b>합니다.
 ...
 - ### 프로필 조회, 수정
   - 회원의 프로필을 조회할 수 있습니다. 현재 회원의 프로필일 경우 <b>프로필 수정/회원 탈퇴</b>를 할 수 있고
   - 수정에서 유효성 검증은 회원가입과 동일합니다.
  ...
  - ### 팔로우, 팔로우 추천 리스트
    - 다른 회원의 프로필일 경우 <b>팔로우/언팔로우</b>를 할 수 있고 팔로우를 하게 되면, 

      팔로우 추천 리스트에 <b>팔로우한 회원을 팔로우한 회원들 또는 팔로우한 회원이 팔로우한 회원 기준으로 추천</b>합니다.
  ...
  - ### 메뉴 랜덤 추첨, CRUD
    - 메인은 <b>현재 회원이 등록했던 메뉴들을 페이징 조회</b>, 회원이 원하는 <b>메뉴를 등록</b>, <b>등록했던 메뉴를 수정/삭제</b>할 수 있습니다.
    - 모든 회원들이 등록했던 메뉴들 중 <b>메뉴 카테고리를 선택하고 랜덤으로 추첨</b>할 수 있습니다.
  ...
  - ### 실시간 1 대 1 채팅
    - 상대 회원 프로필에서 채팅하기를 클릭하면 <b>채팅방이 생성되어 실시간으로 채팅을 주고받을 수 있고 채팅 내역은 DB에 저장</b>합니다. 
    - 1명이 채팅방을 퇴장한 후 <b>상대방이 채팅방을 퇴장하지 않았으면 메시지 전송 시 퇴장한 유저를 다시 기존 채팅방으로 초대</b>합니다.
  ...
  - ### Admin 페이지
    - 관리자 역할을 부여받은 회원은 <b>회원, 게시판, 댓글, 메뉴, 채팅방, 채팅 메시지를 관리</b>할 수 있습니다.
...
### 1. Slice 페이징의 한계
- 무한 스크롤 기능에서 사용한 Slice 페이징은 현재 받아온 엔티티의 id보다 <(작은 것)부터 가져와야 하는데 그렇게 되면 첫 페이지의 첫 데이터를 가져오지 못합니다.
그렇다고 <=(작거나 같음)으로 설정하면 다음 페이지에서 중복되는 데이터가 한 번 더 나오게 됩니다.
- 제가 생각한 방안은 <b>첫 페이지인지 판단하는 필드로 설정</b>해서 각 상황에 따른 첫 페이지 파라미터를 갱신해가며 <b>첫 페이지일 때는 <=, 첫 페이지가 아니면 <으로 모든 데이터를 가져올 수 있게 하였습니다.</b>

### 2. JSON 양방향으로 매핑된 엔티티 필드의 무한 참조 발생
- Ajax로 API를 호출할 때 해당 조회하는 엔티티에 <b>양방향으로 매핑된 엔티티로 인해 서로 참조 되어 무한 참조 문제가 발생</b>하였습니다.
- 먹스타그램의 게시글 엔티티를 불러올 때는 회원 엔티티 필드의 정보가 필요가 없어 회원 엔티티를 @JsonIgnore로 간단히 방지할 수 있었지만, 회원 엔티티를 불러오는 관리자 페이지에서는 회원과 양방향으로 매핑된 게시글, 프로필, 메뉴, 채팅방 엔티티들의 정보가 필요하여
JSON을 호출할 때 <b>DTO를 활용하여 필요한 API 스펙으로 변경하고 호출</b>하였습니다.

### 3. QueryDsl 랜덤 선택의 한계
- <b>QueryDsl에서는 랜덤으로 SELECT 하는 기능이 지원하지 않습니다.</b>
- 그래서 <b>QueryDsl로 관련 리스트를 가져오고 Java의 Random()함수를 활용</b>하여 해당 리스트 중에서 랜덤으로 선택하게 하였습니다.

### 4. 새로운 기술 습득 및 극복
- 처음 접해보는 기술로 실시간 채팅에서 사용되는 Websocket, Stomp의 원리를 이해했지만 코드의 정보가 이해가 되지 않았습니다. 
코드는 실시간 채팅 위주였고 저는 DB에 채팅 내역을 저장 및 출력까지 구현해야 했기에 사이클을 명확히 이해했어야 했습니다.
- 그래서 <b>코드를 반복적으로 리뷰</b>하였고 <b>소켓 연결 후 채팅방 생성, 사용자 입장/대화/퇴장(소켓 끊기)의 사이클 흐름을 따라가는 과정을 통해 구현하고 싶었던 프로젝트에 맞춰서 적용</b>시켰습니다.
...
- 구현 중 막히는 상황이 많았지만, <b>포기하지 않고 끝까지 몰두하는 과정을 통해 문제해결 능력을 기를 수 있었음</b>.
- AWS 환경으로 마이그레이션을 하고 서비스를 CI/CD 파이프라인을 통해 배포하면서 <b>AWS의 각 서비스들을 어떻게 사용하고 효율성을 올려주는 자동화의 필요성을 알 수 있었음</b>.
